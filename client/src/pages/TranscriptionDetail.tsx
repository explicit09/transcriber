...
export default function TranscriptionDetail() {
  ...
  const { data: collabToken } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        `/api/transcriptions/${id}/collab-token?scopes=read,write`,
      );
  ...
  const deleteTranscription = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "There was a problem deleting the transcription. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsDeleting(false);
  ...
  const generateSummaryMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description:
          "The transcript summary and action items have been generated successfully.",
      });
  ...
  const handleDownload = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `${transcription?.fileName || `transcription-${id}`}.pdf`,
      );
  ...
  const mergeSpeakersMutation = useMutation({
    mutationFn: async (targetSpeakerCount: number) => {
      ...
      const response = await apiRequest(
        "POST",
        `/api/transcriptions/${id}/merge-speakers`,
        {
          targetSpeakerCount,
        },
      );
    ...
    onError: (error) => {
      ...
      toast({
        ...
        description:
          "There was a problem merging the speakers. Please try again.",
        variant: "destructive",
      });
    },
    onSettled: () => {
      ...
      <div className="flex flex-col items-center justify-center h-[calc(100vh-200px)]">
        <div className="p-10 bg-white rounded-xl shadow-lg flex flex-col items-center">
          ...
          <p className="text-gray-500 text-sm mt-2">
            Please wait while we prepare your transcript
          </p>
        </div>
      </div>
    );
          ...
          <CardContent className="pt-6">
            <p className="text-red-600 bg-red-50 p-4 rounded-md border border-red-100">
              {error instanceof Error
                ? error.message
                : "The transcription could not be found or may have been deleted."}
            </p>
          </CardContent>
                ...
                <div className="text-gray-500 flex items-center mt-2 text-sm">
                  ...
                  {transcription.duration && (
                    <span className="flex items-center ml-4">
                      ...
                      {String(Math.floor(transcription.duration % 60)).padStart(
                        2,
                        "0",
                      )}
                    </span>
          ...
          <TabsContent value="view" className="mt-4">
            <Card className="border-0 shadow-md overflow-hidden">
              ...
              <CardHeader className="bg-white border-b">
                ...
                <CardDescription>
                  View the transcription content with timestamps and speaker
                  labels.
                </CardDescription>
              </CardHeader>
          ...
          <TabsContent value="speakers" className="mt-4">
            {transcription.structuredTranscript && (
              <SpeakerSimilarity
                ...
                onMergeSpeakers={(targetCount) =>
                  mergeSpeakersMutation.mutateAsync(targetCount)
                }
              />
            )}
          </TabsContent>
